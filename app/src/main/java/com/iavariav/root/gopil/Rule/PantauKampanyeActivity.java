...
public class PantauKampanyeActivity extends AppCompatActivity {

    private LinearLayout divPantauKampanye;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pantau_kampanye);
        initView();
    ...
    private void getDataVideo() {
        final ProgressDialog loading = ProgressDialog.show(PantauKampanyeActivity.this, "Loading", "Mengambil data...", false, false);
        ...
        call.enqueue(new Callback<ArrayList<IklanMasyarkatModel>>() {
            ...
            public void onResponse(Call<ArrayList<IklanMasyarkatModel>> call, Response<ArrayList<IklanMasyarkatModel>> response) {
                ...
                for (IklanMasyarkatModel s : iklanMasyarkatModels){
                    if (s.getJenis_video() != null && s.getJenis_video().contains("Streaming")){
                        judul = s.getJudul();
                        sumber = s.getSumber();
                        gambar = s.getGambar();
                        ...
                        Glide.with(PantauKampanyeActivity.this).load(gambar).centerCrop().error(R.drawable.logo).into(ivRowIklan);
                        ...
                        divPantauKampanye.addView(view);
                        loading.dismiss();
                    }
                }
            }
            ...
            public void onFailure(Call<ArrayList<IklanMasyarkatModel>> call, Throwable t) {
                loading.dismiss();
                Toast.makeText(PantauKampanyeActivity.this, "" + Config.ERROR_NETWORK, Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void initView() {
        divPantauKampanye = (LinearLayout) findViewById(R.id.divPantauKampanye);
    }
}
